...
{{/*
Executor - Gandiva Cache Enabled?
*/}}
{{- define "dremio.executor.gandivaCacheEnabled" -}}
{{- $context := index . 0 -}}
{{- $engineName := index . 1 -}}
{{- $engineConfiguration := default (dict) (get (default (dict) $context.Values.executor.engineOverride) $engineName) -}}
{{- $gandivaCacheEnabledConfig := default (dict) $engineConfiguration.gandivaCache -}}
{{- $gandivaCacheEnabled := default $context.Values.executor.gandivaCache.enabled $gandivaCacheEnabledConfig.enabled -}}
{{- $gandivaCacheEnabled -}}
{{- end -}}

{{/*
Executor - Gandiva Cache Size
*/}}
{{- define "dremio.executor.gandivaCacheSize" -}}
{{- $context := index . 0 -}}
{{- $engineName := index . 1 -}}
{{- $engineConfiguration := default (dict) (get (default (dict) $context.Values.executor.engineOverride) $engineName) -}}
{{- $gandivaCacheSizeConfig := default (dict) $engineConfiguration.gandivaCache -}}
{{- $gandivaCacheSize := int (default $context.Values.executor.gandivaCache.size $gandivaCacheSizeConfig.size) -}}
{{- $gandivaCacheSize -}}
{{- end -}}

{{/*
Executor - Gandiva Cache prewarmFile
*/}}
{{- define "dremio.executor.gandivaCache.prewarmFile" -}}
{{- $context := index . 0 -}}
{{- $engineName := index . 1 -}}
{{- $engineConfiguration := default (dict) (get (default (dict) $context.Values.executor.engineOverride) $engineName) -}}
{{- $gandivaCachepreWarmFileConfig := default (dict) $engineConfiguration.gandivaCache -}}
{{- $gandivaCachepreWarmFile := default $context.Values.executor.gandivaCache.prewarmFile $gandivaCachepreWarmFileConfig.prewarmFile -}}
{{- $gandivaCachepreWarmFile -}}
{{- end -}}

{{/*
Executor - Gandiva Cache Peristent Volume Claims
*/}}
{{- define "dremio.executor.gandivaCache.volumeClaimTemplate" -}}
{{- $context := index . 0 -}}
{{- $engineName := index . 1 -}}
{{- $engineConfiguration := default (dict) (get (default (dict) $context.Values.executor.engineOverride) $engineName) -}}
{{- $engineGandivaCacheConfig := default (dict) $engineConfiguration.gandivaCache -}}
{{- $gandivaCacheConfig := coalesce $engineConfiguration.gandivaCache $context.Values.executor.gandivaCache -}}
{{- $gandivaCacheStorageClass := coalesce $engineGandivaCacheConfig.storageClass $context.Values.executor.gandivaCache.storageClass $engineConfiguration.storageClass $context.Values.executor.storageClass $context.Values.storageClass -}}
{{- if $gandivaCacheConfig.enabled -}}
{{- range $index, $gandivaCacheVolumeConfig := $gandivaCacheConfig.volumes }}
{{- $volumeStorageClass := coalesce $gandivaCacheVolumeConfig.storageClass $gandivaCacheStorageClass }}
- metadata:
    name: {{ coalesce $gandivaCacheVolumeConfig.name (printf "dremio-%s-executor-gandiva-%d" $engineName $index) }}
  spec:
    accessModes: ["ReadWriteOnce"]
    {{- if $volumeStorageClass }}
    storageClassName: {{ $volumeStorageClass }}
    ...
    resources:
      requests:
        storage: {{ $gandivaCacheVolumeConfig.size }}
{{- end -}}
{{- end -}}
{{- end -}}

{{/*
Executor - Gandiva Cache Peristent Volume Mounts
*/}}
{{- define "dremio.executor.gandivaCache.volumeMounts" -}}
{{- $context := index . 0 -}}
{{- $engineName := index . 1 -}}
{{- $engineConfiguration := default (dict) (get (default (dict) $context.Values.executor.engineOverride) $engineName) -}}
{{- $gandivaCacheConfig := coalesce $engineConfiguration.gandivaCache $context.Values.executor.gandivaCache -}}
...
{{- range $index, $gandivaCacheVolumeConfig := $gandivaCacheConfig.volumes }}
- name: {{ coalesce $gandivaCacheVolumeConfig.name (printf "dremio-%s-executor-gandiva-%d" $engineName $index) }}
  mountPath: /opt/dremio/gandivacache/c{{ $index }}
{{- end -}}
{{- end -}}
{{- end -}}
...
Executor - Gandiva Cache Peristent Volume Mounts
*/}}
{{- define "dremio.executor.gandivaCache.initContainers" -}}
{{- $context := index . 0 -}}
{{- $engineName := index . 1 -}}
{{- $engineConfiguration := default (dict) (get (default (dict) $context.Values.executor.engineOverride) $engineName) -}}
{{- $gandivaCacheConfig := coalesce $engineConfiguration.gandivaCache $context.Values.executor.gandivaCache -}}
{{- if $gandivaCacheConfig.enabled -}}
- name: chown-gandiva-cache-directory
  image: {{ $context.Values.image }}:{{ $context.Values.imageTag }}
  imagePullPolicy: IfNotPresent
  securityContext:
    runAsUser: 0
  volumeMounts:
  {{- include "dremio.executor.gandivaCache.volumeMounts" (list $context $engineName) | nindent 2 }}
  command: ["bash"]
  args: ["-c", "rm -rf /opt/dremio/gandivacache/c*/*; chown -R dremio:dremio /opt/dremio/gandivacache/c*"]
- name: copy-gandiva-cache
  image: amazon/aws-cli:latest
  imagePullPolicy: IfNotPresent
  volumeMounts:
  {{- include "dremio.executor.gandivaCache.volumeMounts" (list $context $engineName) | nindent 2 }}
  command: ["bash"]
  args: ['-c', 'aws s3 cp {{- include "dremio.executor.gandivaCache.prewarmFile" (list $context $engineName) | indent 1 }} /opt/dremio/gandivacache/c0/gandiva-cache.tar'] 
- name: untar-gandiva-cache
  image: {{ $context.Values.image }}:{{ $context.Values.imageTag }}
  imagePullPolicy: IfNotPresent
  volumeMounts:
  {{- include "dremio.executor.gandivaCache.volumeMounts" (list $context $engineName) | nindent 2 }}
  command: ["bash"]
  args: ["-c", "tar -xf /opt/dremio/gandivacache/c0/gandiva-cache.tar -C /opt/dremio/gandivacache/c0; rm -f /opt/dremio/gandivacache/c0/*.tar"]
{{- end -}}
{{- end -}}
